...
//                    defineTimePeriod(selectionScreen, validator);
	...
	private void calculateSpending() {
		Path filePath = Paths.get("/home/aren_rend/Programs/1_Intellij_Idea/Projects/Spending_Console/SaveInformation.txt");
		try(BufferedReader bufferedReader = new BufferedReader(new FileReader(filePath.toFile()))) {
			String currentLine;
			int allSpends = 0;
			while((currentLine = bufferedReader.readLine()) != null) {
					Pattern pattern = Pattern.compile("(\\d+)\\s*rub");
					Matcher matcher = pattern.matcher(currentLine);
					if(matcher.find()) {
						allSpends += Integer.parseInt(matcher.group(1));
					}

			}
			System.out.println(allSpends);
		} catch (Exception e) {
			throw new RuntimeException("Not found File: " + e.getMessage(), e);
		}
	}
